...
test("parseSmokeArgs defaults", () => {
  assert.deepEqual(parseSmokeArgs([], {}), {
    ...
    // Derived from cwd, not a hardcoded /workspace, so the default resolves
    // inside whichever checkout is under test.
    outPng: resolve(process.cwd(), "screenshots", "app-builder-preview.png"),
    baseline: "",
  });
});

test("parseSmokeArgs default output stays inside the project root", () => {
  const { outPng } = parseSmokeArgs([], {});
  assert.ok(outPng.startsWith(process.cwd() + sep), `${outPng} is outside ${process.cwd()}`);
  assert.doesNotMatch(outPng, /\/workspace\//);
});
...
test("browser-smoke wires the guard and verdict helpers", () => {
  ...
  assert.match(src, /const outputRoots = \[process\.cwd\(\)\]/);
  ...
  assert.doesNotMatch(
    src,
    /\["\/workspace"\]/,
    "hardcoded /workspace roots break outside the platform sandbox",
  );
